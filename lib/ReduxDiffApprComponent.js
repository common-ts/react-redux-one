...
var ReduxDiffApprComponent = (function(_super){
  ...
  function ReduxDiffApprComponent(props, metadata, resourceService, showMessage, showError, loading){
    var _this = _super.call(this, props, react_onex_1.buildKeys(metadata), resourceService, showMessage, showError, loading) || this;
  ...
  ReduxDiffApprComponent.prototype.load=function(_id){
    ...
    if (id && id !== ''){
      var ctx = {};
      this.id = _id;
      this.props.diff({ parameter: id, ctx: ctx, callback: { execute: this.showDiff, handleError: this.handleError, formatData: this.formatDiffModel } });
    }
  };
  ReduxDiffApprComponent.prototype.approve=function(event){
    ...
    var ctx = {};
    var id = this.id;
    this.props.approve({ parameter: id, ctx: ctx, callback: { execute: this.postApprove, handleError: this.handleError } });
  };
  ReduxDiffApprComponent.prototype.reject=function(event){
    event.preventDefault();
    var ctx = {};
    var id = this.id;
    this.props.reject({ parameter: id, ctx: ctx, callback: { execute: this.postReject, handleError: this.handleError } });
  };
