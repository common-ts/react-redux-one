...
var ReduxEditComponent = (function(_super){
  ...
  function ReduxEditComponent(props, metadata, resourceService, ui, showMessage, showError, confirm, getLocale, loading, patchable, backOnSaveSuccess){
    var _this = _super.call(this, props, metadata, resourceService, ui, showMessage, showError, confirm, getLocale, loading, patchable, backOnSaveSuccess) || this;
  ...
  ReduxEditComponent.prototype.componentDidMount = function(){
    this.form = react_onex_1.initForm(this.ref.current, this.ui.initMaterial);
  ...
  ReduxEditComponent.prototype.save = function(parameter, body, isBack){
    ...
    if (!this.newMode){
      if (this.patchable === true && body && Object.keys(body).length > 0){
        this.props.patch({ parameter: body, ctx: ctx, callback: { execute: this.postSave, handleError: this.handleError } });
      }
      else {
        this.props.update({ parameter: parameter, ctx: ctx, callback: { execute: this.postSave, handleError: this.handleError } });
      }
    }
    else {
      this.props.insert({ parameter: parameter, ctx: ctx, callback: { execute: this.postSave, handleError: this.handleError } });
    }
  };
...
var ReduxSearchComponent = (function(_super){
  ...
  function ReduxSearchComponent(props, resourceService, ui, showMessage, showError, getLocale, loading, autoSearch, listFormId){
